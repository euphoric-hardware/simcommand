...
class NeuromorphicProcessorRawSimulatorTester extends NeuromorphicProcessorTester {
  ...
  def runTest(dut: SimulatorContext): Long = {
    var cycles = 0
    // Reset inputs
    dut.poke("io_io_uartRx", 1)
    assert(dut.peek("io_io_uartTx") == 1)
    dut.poke("reset", 1)
    dut.step()
    cycles += 1
    dut.poke("reset", 0)
  ...
  it should "process an image" taggedAs(Verilator) in {
    ...
    val r = stage.run(Seq(
      ChiselGeneratorAnnotation(() => new NeuromorphicProcessorBBWrapper()),
      targetDir,
    ))
  ...
  def step(dut: SimulatorContext): Unit = {
    if (delay_count > 0) {
      ...
      state = if (state == 0) {
        ...
        byte = (dut.peek("io_io_uartTx").toInt << (state - 1)) | byte
        delay_count = bitDelay - 1
        state + 1
      } else {
        assert(state == 9)
        bytes.enqueue(byte)
        assert(dut.peek("io_io_uartTx") == 1, "stop bit")
        0
      }
    }
  }
