 ...
 * Modified: Anthon V. Riber
 * 
 * Free to use license: https://github.com/freechipsproject/ip-contributions
 */
...
class UartMain(frequency: Int, baudRate: Int) extends Module {
  ...
  io.txd := tx.io.txd
  io.txReady := tx.io.channel.ready
  tx.io.channel.valid := io.txValid
  tx.io.channel.bits := io.txByte

  rx.io.rxd := io.rxd
  rx.io.channel.ready := io.rxReady
  io.rxValid := rx.io.channel.valid
  io.rxByte := rx.io.channel.bits
