...
class NeuromorphicProcessor(synth: Boolean = false) extends Module {
  ...
  inC0Mem.io.clka  := topClock.asBool
  inC0Mem.io.clkb  := coreClock.asBool
  inC0Mem.io.web   := false.B
  inC0Mem.io.dib   := 0.U
  
  val inC1Mem = Module(TrueDualPortMemory(RATEADDRWIDTH+1, RATEWIDTH, synth))
  inC1Mem.io.clka  := topClock.asBool
  inC1Mem.io.clkb  := coreClock.asBool
  inC1Mem.io.web   := false.B
  inC1Mem.io.dib   := 0.U
  
  val outMem = Module(TrueDualPortFIFO(16, 8, synth))
  outMem.io.clki   := coreClock.asBool
  outMem.io.clko   := topClock.asBool
  ...
  // The following is driven by the global clock
  val newTS = Wire(Bool())
  withClock(topClock) {
...
object NeuromorphicProcessor extends App {
  (new chisel3.stage.ChiselStage).emitVerilog(new NeuromorphicProcessor(true), Array("--target-dir", "build"))
}
